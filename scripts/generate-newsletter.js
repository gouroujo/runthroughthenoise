...
function getContentSince(lastRunDate) {
  ...
  for (const item of allMetadata) {
    ...
    try {
      ...
      console.error(
        `Error processing content item ${item.slug}:`,
        error.message,
      )
    }
  }
...
async function generateNewsletter(newContent, lastRunDate) {
  ...
  const hasNewContent = Object.values(newContent).some(
    (items) => items.length > 0,
  )

  if (!hasNewContent) {
    console.log(
      'No new content found since last run. Skipping newsletter generation.',
    )
    return
  }
  ...
  console.log(
    `Total items: ${Object.values(newContent).reduce((sum, items) => sum + items.length, 0)}`,
  )
}
